      ...
      switch (b.type) {
        ...
        case "gallery": {
          ...
          if (layout === "side" && imgs.length >= 2) {
            ...
            return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="lv-gallery" style="margin:20px 0"><tr>${two.map((im: any) => `<td class="lv-gallery-cell" width="50%" style="padding:4px;vertical-align:top"><img src="${esc(im.url)}" alt="${esc(im.alt || "")}" style="width:100%;height:auto;border-radius:4px;display:block"/>${im.caption ? `<div style="font-size:12px;color:${p.muted};text-align:center;margin-top:6px">${esc(im.caption)}</div>` : ""}</td>`).join("")}</tr></table>`;
          }
          const rows: any[][] = [];
          for (let i = 0; i < imgs.length; i += 2) rows.push(imgs.slice(i, i + 2));
          return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="lv-gallery" style="margin:20px 0">${rows.map(r => `<tr>${r.map((im: any) => `<td class="lv-gallery-cell" width="50%" style="padding:4px;vertical-align:top"><img src="${esc(im.url)}" alt="${esc(im.alt || "")}" style="width:100%;height:auto;border-radius:4px;display:block"/>${im.caption ? `<div style="font-size:12px;color:${p.muted};text-align:center;margin-top:6px">${esc(im.caption)}</div>` : ""}</td>`).join("")}${r.length === 1 ? '<td width="50%"></td>' : ""}</tr>`).join("")}</table>`;
        }
        case "social": {
